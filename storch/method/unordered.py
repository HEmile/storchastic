...
class UnorderedSetEstimator(SampleWithoutReplacementMethod):
    ...
    def plate_weighting(
        ...
        log_v = (
            torch.arange(1, self.num_int_points, out=log_probs.new())
            / self.num_int_points
        ).log()
        ...
        g_bound = log_probs + self.a + torch.log(-log_v)
        ...
        integrand = sum_of_terms + torch.expm1(self.a + phi_D_min_S) * log_v
        ...
        if self.use_baseline:
            ...
            skip_diag = 1 - torch.eye(plate.n, out=log_probs.new())
            integrand_without_ss = integrand_without_s - terms * skip_diag

            log_p_S_without_ss = integrand_without_ss.logsumexp(dim=-1)

            log_snd_leave_one_out = log_p_S_without_ss - log_p_S_without_s
            plate.log_snd_leave_one_out = log_snd_leave_one_out

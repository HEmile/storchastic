...
def _convert_indices(tensor: storch.Tensor, dims=List[Union[str, int]]) -> (List[int], List[str]):
    conv_indices = []
    red_batches = []
    for index in dims:
        if isinstance(index, int):
            if index >= tensor.batch_dims or index < 0 and index >= -tensor.event_dims:
                conv_indices.append(index)
            else:
                print(tensor.shape, index)
                raise IndexError("Can only pass indexes for event dimensions." + str(tensor) + ". Index: " + str(index))
        else:
            conv_indices.append(tensor.get_batch_dim_index(index))
